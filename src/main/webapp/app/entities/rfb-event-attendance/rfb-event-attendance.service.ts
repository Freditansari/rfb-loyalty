...
export class RfbEventAttendanceService {
    ...
    search(req?: any): Observable<HttpResponse<RfbEventAttendance[]>> {
        const options = createRequestOption(req);
        return this.http.get<RfbEventAttendance[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<RfbEventAttendance[]>) => this.convertArrayResponse(res));
    }
